# MMSA 通用训练测试指南

## 📋 概述

`train.py` 是一个通用的训练和测试脚本，支持配置任意模型和数据集进行训练和测试。

---

## 🚀 快速开始

### 基本用法

```bash
# 使用 TFN 模型训练 copa_1231 数据集
python train.py -m tfn -d copa_1231

# 使用 LMF 模型训练 MOSI 数据集
python train.py -m lmf -d mosi

# 使用多个随机种子
python train.py -m tfn -d copa_1231 -s 1111 1112 1113
```

---

## 📝 支持的模型

所有 `singleTask` 目录下的模型：

- `tfn` - Tensor Fusion Network
- `lmf` - Low-rank Multimodal Fusion
- `mfn` - Memory Fusion Network
- `graph_mfn` - Graph Memory Fusion Network
- `ef_lstm` - Early Fusion LSTM
- `lf_dnn` - Late Fusion DNN
- `mult` - Multimodal Transformer
- `misa` - Modality-Invariant and -Specific Representations
- `bert_mag` - BERT Multimodal Adaptation Gate
- `mfm` - Multimodal Factorization Model
- `mmim` - Multimodal Mutual Information Maximization
- `mctn` - Multimodal Cyclic Translation Network
- `cenet` - Cross-modal Enhancement Network
- `almt` - Adaptive Learning Multimodal Transformer
- `almt_cider` - ALMT with CIDer

---

## 📊 支持的数据集

- `mosi` - CMU-MOSI
- `mosei` - CMU-MOSEI
- `sims` - CH-SIMS
- `simsv2` - CH-SIMS v2
- `custom` - 自定义数据集
- `train_12_16` - 训练数据集
- `copa_1231` - COPA 1231 数据集

---

## 🔧 命令行参数

### 必需参数

- `-m, --model`: 模型名称（从支持的模型列表中选择）
- `-d, --dataset`: 数据集名称（从支持的数据集列表中选择）

### 可选参数

#### 训练配置

- `-s, --seeds`: 随机种子列表（默认: `[1111]`）
  ```bash
  python train.py -m tfn -d copa_1231 -s 1111 1112 1113
  ```

- `--skip-validation`: 跳过验证集流程（直接训练和测试）
  ```bash
  python train.py -m tfn -d copa_1231 --skip-validation
  ```

#### GPU/CPU 配置

- `--gpu-ids`: GPU ID 列表（默认: 自动检测并使用第一个GPU）
  ```bash
  # 使用 GPU 0
  python train.py -m tfn -d copa_1231 --gpu-ids 0
  
  # 使用多个 GPU
  python train.py -m tfn -d copa_1231 --gpu-ids 0 1
  
  # 强制使用 CPU
  python train.py -m tfn -d copa_1231 --gpu-ids ""
  ```

#### 数据加载配置

- `--num-workers`: 数据加载器工作进程数（默认: GPU模式4, CPU模式2）
  ```bash
  python train.py -m tfn -d copa_1231 --num-workers 8
  ```

#### 自定义配置

- `--config`: 自定义配置文件路径（JSON格式）
  ```bash
  python train.py -m tfn -d copa_1231 --config my_config.json
  ```

  配置文件示例 (`config_example.json`):
  ```json
  {
    "batch_size": 16,
    "learning_rate": 0.0005,
    "early_stop": 10,
    "hidden_dims": [128, 32, 128]
  }
  ```

- `--config-file`: MMSA 配置文件路径（覆盖默认的 config_regression.json）
  ```bash
  python train.py -m tfn -d copa_1231 --config-file custom_config.json
  ```

#### 输出目录配置

- `--model-save-dir`: 模型保存目录（默认: `./saved_models`）
- `--res-save-dir`: 结果保存目录（默认: `./results`）
- `--log-dir`: 日志保存目录（默认: `./logs`）

#### 日志配置

- `--verbose-level`: 日志详细程度（0=ERROR, 1=INFO, 2=DEBUG，默认: 1）

#### 超参数调优

- `--tune`: 启用超参数调优模式
- `--tune-times`: 超参数调优次数（默认: 50）
  ```bash
  python train.py -m tfn -d copa_1231 --tune --tune-times 100
  ```

---

## 📖 使用示例

### 示例 1: 基本训练

```bash
# 使用 TFN 模型训练 copa_1231 数据集
python train.py -m tfn -d copa_1231
```

### 示例 2: 多随机种子训练

```bash
# 使用 3 个随机种子，结果取平均值
python train.py -m tfn -d copa_1231 -s 1111 1112 1113
```

### 示例 3: 跳过验证集

```bash
# 直接训练和测试，跳过验证集
python train.py -m tfn -d copa_1231 --skip-validation
```

### 示例 4: 使用自定义配置

```bash
# 创建配置文件 my_config.json
cat > my_config.json << EOF
{
  "batch_size": 16,
  "learning_rate": 0.0005,
  "early_stop": 15
}
EOF

# 使用自定义配置训练
python train.py -m tfn -d copa_1231 --config my_config.json
```

### 示例 5: CPU 训练

```bash
# 强制使用 CPU
python train.py -m tfn -d copa_1231 --gpu-ids ""
```

### 示例 6: 尝试不同模型

```bash
# 尝试 LMF 模型
python train.py -m lmf -d copa_1231

# 尝试 MISA 模型
python train.py -m misa -d copa_1231

# 尝试 ALMT 模型
python train.py -m almt -d copa_1231
```

### 示例 7: 超参数调优

```bash
# 进行 100 次超参数调优
python train.py -m tfn -d copa_1231 --tune --tune-times 100
```

### 示例 8: 完整配置示例

```bash
python train.py \
  -m tfn \
  -d copa_1231 \
  -s 1111 1112 1113 \
  --gpu-ids 0 \
  --num-workers 4 \
  --skip-validation \
  --config my_config.json \
  --model-save-dir ./my_models \
  --res-save-dir ./my_results \
  --log-dir ./my_logs \
  --verbose-level 2
```

---

## 📂 输出文件

训练完成后，结果保存在以下位置：

### 模型文件
```
saved_models/{model_name}-{dataset_name}.pth
例如: saved_models/tfn-copa_1231.pth
```

### 测试结果
```
results/normal/{dataset_name}.csv
例如: results/normal/copa_1231.csv
```

包含所有评估指标：
- 基础指标：准确率、F1分数、MAE、相关系数等
- COPA指标（如果数据集支持）：整体准确率和各范式准确率

### 训练日志
```
logs/{model_name}-{dataset_name}.log
例如: logs/tfn-copa_1231.log
```

---

## 🔍 查看帮助

```bash
python train.py --help
```

会显示完整的参数说明和使用示例。

---

## ⚙️ 配置文件说明

### 自定义配置文件格式

自定义配置文件（`--config`）是一个 JSON 文件，可以覆盖默认的超参数：

```json
{
  "batch_size": 16,
  "learning_rate": 0.0005,
  "early_stop": 10,
  "hidden_dims": [128, 32, 128],
  "text_out": 32,
  "post_fusion_dim": 64,
  "dropouts": [0.3, 0.3, 0.3, 0.5]
}
```

**注意**: 不同模型支持的配置项可能不同，请参考各模型的配置文件。

### MMSA 配置文件

MMSA 框架使用 `src/MMSA/config/config_regression.json` 作为默认配置文件。该文件包含：
- 数据集配置（特征路径、序列长度、特征维度等）
- 模型配置（各模型在不同数据集上的超参数）

如果需要修改，可以直接编辑该文件，或使用 `--config-file` 指定新的配置文件。

---

## 🎯 最佳实践

### 1. 首次训练

```bash
# 使用默认配置快速验证
python train.py -m tfn -d copa_1231 -s 1111
```

### 2. 正式训练

```bash
# 使用多个随机种子，获得更稳定的结果
python train.py -m tfn -d copa_1231 -s 1111 1112 1113 1114 1115
```

### 3. 模型对比

```bash
# 训练多个模型进行对比
python train.py -m tfn -d copa_1231
python train.py -m lmf -d copa_1231
python train.py -m misa -d copa_1231
```

### 4. 超参数调优

```bash
# 先进行超参数调优，找到最佳配置
python train.py -m tfn -d copa_1231 --tune --tune-times 50

# 然后使用最佳配置进行正式训练
python train.py -m tfn -d copa_1231 --config best_config.json
```

---

## 🐛 故障排查

### 问题 1: 模型名称错误

**错误**: `ValueError: invalid choice: 'xxx'`

**解决**: 检查模型名称是否正确，使用 `python train.py --help` 查看支持的模型列表。

### 问题 2: 数据集不存在

**错误**: `FileNotFoundError: xxx.pkl`

**解决**: 
- 检查数据集文件是否存在
- 确认数据集名称是否正确
- 对于 `copa_1231`，确保已运行数据转换脚本

### 问题 3: GPU 内存不足

**解决**:
- 减小 batch_size（在配置文件中）
- 使用 CPU 训练：`--gpu-ids ""`
- 减小 num_workers

### 问题 4: COPA 指标未出现

**检查**:
1. 确认数据集名称为 `copa_1231`
2. 检查日志中是否有 "COPA评估失败" 的警告
3. 确认测试集样本数能被 122 整除

---

## 📚 相关文档

- `训练测试流程说明.md` - 详细的训练测试流程说明
- `训练测试指南.md` - COPA 1231 数据集专用指南
- `src/MMSA/config/config_regression.json` - 默认配置文件

---

**最后更新**: 2025-01-03


